...
impl<'a> JackCompiler<'a> {
    ...
    fn parse_let_statement(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        self.vm_output += &format!("{:indent$}<keyword> let </keyword>\n", "", indent=xml_indent+2);
        self.token_iterator.next();
    ...
    fn parse_if_statement(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        self.vm_output        += &format!("{:indent$}<keyword> if </keyword>\n", "", indent=xml_indent+2);
        self.token_iterator.next();
    ...
    fn parse_while_statement(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        self.vm_output        += &format!("{:indent$}<keyword> while </keyword>\n", "", indent=xml_indent+2);
        self.token_iterator.next();
    ...
    fn parse_do_statement(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        self.vm_output        += &format!("{:indent$}<keyword> do </keyword>\n", "", indent=xml_indent+2);
        self.token_iterator.next();
    ...
    fn parse_return_statement(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        self.vm_output += &format!("{:indent$}<keyword> return </keyword>\n", "", indent=xml_indent+2);
        self.token_iterator.next();
    ...
    fn parse_term(&mut self, xml_indent : usize) -> Result<(), &'static str> {
        ...
        match self.token_iterator.peek().unwrap() {
            Token::IntConstant(i)           => {
                self.vm_output += &format!("{:indent$}<integerConstant> {i:} </integerConstant>\n", "", indent=xml_indent+2, i=i);
                self.token_iterator.next();
            },
            Token::StringConstant(s)        => {
                self.vm_output += &format!("{:indent$}<stringConstant> {s:} </stringConstant>\n", "", indent=xml_indent+2, s=s);
                self.token_iterator.next();
            },
            Token::Keyword(Keyword::True)   => {
                self.vm_output += &format!("{:indent$}<keyword> true </keyword>\n", "", indent=xml_indent+2);
                self.token_iterator.next();
            },
            Token::Keyword(Keyword::False)  => {
                self.vm_output += &format!("{:indent$}<keyword> false </keyword>\n", "", indent=xml_indent+2);
                self.token_iterator.next();
            },
            Token::Keyword(Keyword::Null)   => {
                self.vm_output += &format!("{:indent$}<keyword> null </keyword>\n", "", indent=xml_indent+2);
                self.token_iterator.next();
            },       
            Token::Keyword(Keyword::This)   => {
                self.vm_output += &format!("{:indent$}<keyword> this </keyword>\n", "", indent=xml_indent+2);
                self.token_iterator.next();
            },
            // (expression)
            Token::Symbol('(')              =>   {
            ...
            Token::Identifier(name)      => {
                self.vm_output += &format!("{:indent$}<identifier> {name} </identifier>\n", "", indent=xml_indent+2, name=name);
                self.token_iterator.next();
                match **self.token_iterator.peek().unwrap() {
                    // varName[expression]
                    Token::Symbol('[') => {
